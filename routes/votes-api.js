    ...
    this._db
      ...
      .then((result) => {
        if (result === undefined) {
          ...
          Number(loginInfo.loggedInUser.id) === Number(result.voter_id)
        ) {
          callback(result);
          return true;
        } else {
  ...
  createRoutes = function () {
      ...
      runWithLoginUser(req.session, req.session.user_id, (loginInfo) => {
        if (loginInfo.loggedInUser) {
          ...
          if (Number.isNaN(userIdNum)) {
            ...
            this._db
              .getVotesByUser(req.params.id, req.params.userId)
              .then((result) => {
                res.json(result);
              })
              .catch((error) => {
    ...
    this._router.post(`/new`, (req, res) => {
      runWithLoginUser(req.session, req.session.user_id, (loginInfo) => {
        if (loginInfo.loggedInUser) {
          if (
            Number(loginInfo.loggedInUser.id) === Number(req.session.user_id)
          ) {
            this._db
              .insertVote(req.body)
              .then((result) => {
            ...
            sendJsonErrorMessage(
              res,
              500,
              "logged in user does not match voter id."
            );
          }
        } else {
          sendJsonErrorMessage(res, 401, error);
    ...
    this._router.get(`/:voteId`, (req, res) => {
      runWithLoginUser(req.session, req.session.user_id, (loginInfo) => {
        this.checkValidAPIRequestForUser(
          res,
          loginInfo,
          req.params.voteId,
          (result) => {
